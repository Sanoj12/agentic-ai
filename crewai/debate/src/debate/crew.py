...
class Debate():
    ...
    @task
    def supporter(self) -> Task:
        return Task(
            config=self.tasks_config['supporter'], # type: ignore[index]
        )

    @task
    def challenger(self) -> Task:
        return Task(
            config=self.tasks_config['Challenger'], # type: ignore[index]
